...
class NycEventsProvider(SourceAdapter):
    def acquire(self, source: SourceConfig, region: dict):
        ...
        stamp=datetime.now(timezone.utc).isoformat().replace("+00:00","Z"); geo_token=__import__('os').environ.get("NYC_GEOCLIENT_SUBSCRIPTION_KEY"); out=[]
        for e in raw.get("items",[]):
            ...
            point=self._geocode(address,geo_token)
    ...
    def _geocode(self,address,token):
        if not token: return None
        try:
            url="https://api.nyc.gov/geoclient/v2/search?"+urlencode({"input":address,"exactMatchForSingleSuccess":"true"})
