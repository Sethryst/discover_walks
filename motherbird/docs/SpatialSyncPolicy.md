...
The browser now has a local-only `spatial_local_operations` outbox and `js/spatial-sync-outbox.js`; neither is wired to an HTTP or Supabase client. `npm run check:spatial-sync -- <region>` fails closed until the regional package declares a human-approved `syncIdentity.poiVersion` and `syncIdentity.boundaryVintage`. Do not invent those labels from build timestamps: they are the county's data-ownership decision.
