...
export async function collectKpiInventory() {
  ...
  for (const [cityId, city] of Object.entries(CITIES)) {
    ...
    const files = [city.dataFile, city.supplementalPoiFile, city.journeyFile, city.civicFile, city.weatherFile].filter(Boolean);
    const missingFiles = [];
    for (const file of files) if (!(await exists(appPath(file)))) missingFiles.push(file);
    const regionId = CITY_REGION_IDS[cityId] || cityId;
    cities.push({
      cityId, regionId, name: city.name, state: city.state,
      poiCount: countPoiRecords(data) + countPoiRecords(supplemental),
      journeyCount: countJourneyRecords(journeys),
      ...
      configuredSources: sources.filter((source) => source.regionId === regionId).length,
      missingFiles,
      status: missingFiles.length ? 'Needs attention' : 'Connected'
    });
  }
  ...
  const gaps = [
    ...cities.filter((city) => city.missingFiles.length).map((city) => ({
      severity: 'Broken reference', region: city.name, detail: city.missingFiles.join(', ')
    })),
    ...configs.filter((region) => !selectableRegionIds.has(region.id)).map((region) => ({
      severity: 'Producer only', region: region.name, detail: 'Configured pipeline exists, but this region is not selectable in Mother Bird.'
    })),
    ...cities.filter((city) => !configuredRegionIds.has(city.regionId)).map((city) => ({
      severity: 'Frontend only', region: city.name, detail: 'Selectable app package has no matching governed producer config.'
    }))
  ];
  ...
  return {
    ...
    summary: {
      ...
      connectedRegions: cities.filter((city) => city.status === 'Connected').length,
...
export function renderKpiHtml(model) {
  const { summary } = model;
  const cards = [
    ['Selectable regions', summary.selectableRegions, `${summary.connectedRegions} have every referenced artifact`],
    ...
    ['Connection gaps', summary.gaps, 'Producer-only, frontend-only, or broken references']
  ];
  ...
  const cityRows = renderRows(model.cities, [
    ...
    { key: 'status', label: 'Status', render: (r) => `<span class="pill ${r.status === 'Connected' ? 'good' : 'warn'}">${escapeHtml(r.status)}</span>` }
  ]);
...
<section class="panel"><h2>App coverage</h2><p>Counts are records the deployed app can load through <code>CITIES</code>. “Connected” means every referenced file exists; it does not mean every possible local source is covered.</p><div class="table-wrap"><table><thead><tr><th>App region</th><th>Places</th><th>Walks/routes</th><th>Civic items</th><th>Conditions</th><th>Producer sources</th><th>Status</th></tr></thead><tbody>${cityRows}</tbody></table></div></section>
<section class="two"><article class="panel"><h2>Provider mix</h2><p>Configured source endpoints by acquisition adapter.</p>${providerBars}</article><article class="panel"><h2>Connection gaps</h2><p>Actionable mismatches between producer configuration and the deployed frontend.</p><table><thead><tr><th>Type</th><th>Region</th><th>What is not connected</th></tr></thead><tbody>${gapRows || '<tr><td colspan="3" class="empty">No connection gaps detected.</td></tr>'}</tbody></table></article></section>
...
<section class="panel"><h2>How to read the KPIs</h2><p class="note"><b>Configured endpoint</b> means a governed source definition exists. <b>Connected region</b> means the frontend’s referenced artifacts exist. <b>Review backlog</b> is discovery-only and never publishes automatically. Record counts are package depth, not completeness or freshness guarantees.</p><p>Rebuild with <code>cd motherbird &amp;&amp; npm run build</code>. The generator re-reads regional configs, CITIES mappings, packaged artifacts, and the review backlog.</p></section>
<footer><a href="../">← Mother Bird</a> · Generated from repository sources</footer></main><script>const q=document.querySelector('#sourceSearch'),p=document.querySelector('#providerFilter'),rows=[...document.querySelectorAll('#sourceTable tbody tr')],empty=document.querySelector('#sourceEmpty');function filter(){const needle=q.value.trim().toLowerCase(),provider=p.value.toLowerCase();let shown=0;for(const row of rows){const ok=(!needle||row.textContent.toLowerCase().includes(needle))&&(!provider||row.children[2].textContent.toLowerCase()===provider);row.hidden=!ok;if(ok)shown++}empty.hidden=shown>0}q.addEventListener('input',filter);p.addEventListener('change',filter)</script></body></html>`;
}
