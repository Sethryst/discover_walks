...
export async function openWalkDetail(id) {
  ...
  const [moments, observations, soundtrack, savedTracks] = await Promise.all([db.all('moments'), db.all('observations'), db.get('walk_soundtracks', id), db.all('radio_saved_tracks')]);
