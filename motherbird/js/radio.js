...
const allSoundcloudPlaylists = () => [...SOUNDCLOUD_PLAYLISTS, ...SOUNDCLOUD_ADDITIONAL_PLAYLISTS].map(([label, url]) => url.endsWith('/gospelandpraise') ? ['Classic Gospel Hymns · Oh the Deep, Deep Love of Jesus', 'https://soundcloud.com/jonny-music-1/once-to-every-man-and-nation-hymns-with-jonny'] : [label, url]);
...
function bindSoundcloudWidget() { const frame = el('radioSoundcloudFrame'); const tracks = el('radioSoundcloudTrackSelect'); const count = el('radioSoundcloudTrackCount'); if (!frame || !tracks || !window.SC?.Widget) return; soundcloudWidget = window.SC.Widget(frame); soundcloudWidget.bind(window.SC.Widget.Events.READY, () => soundcloudWidget.getSounds((sounds) => { const total = sounds?.length || 0; tracks.innerHTML = total ? sounds.map((sound, index) => `<option value="${index}">${escapeHtml(sound.title || `Track ${index + 1}`)}</option>`).join('') : '<option>No tracks reported</option>'; if (count) count.textContent = `${total} track${total === 1 ? '' : 's'} available${total && total < 10 ? ' · fewer than 10 in this source' : ''}`; })); }
function renderSoundcloudPlaylists() { const search = el('radioSoundcloudSearch'); const select = el('radioSoundcloudSelect'); const count = el('radioSoundcloudCount'); if (!select) return; const catalog = allSoundcloudPlaylists(); const query = String(search?.value || '').trim().toLowerCase(); const matches = catalog.filter(([label]) => label.toLowerCase().includes(query)); const selected = select.value; select.innerHTML = matches.map(([label, url]) => `<option value="${escapeHtml(url)}">${escapeHtml(label)}</option>`).join(''); if (matches.some(([, url]) => url === selected)) select.value = selected; if (count) count.textContent = `${matches.length} of ${catalog.length} playlists`; }
...
function bind() {
  ...
  window.addEventListener('walk-ended', async ({ detail }) => { if (!detail?.id) return; const since = walkSoundtrackStartedAt || 0; const trackIds = [...new Set(state.history.filter((event) => event.event === 'played' && event.at >= since).map((event) => event.trackId))]; await db.put('walk_soundtracks', { id: detail.id, walkId: detail.id, trackIds, savedAt: Date.now() }); const tracks = await db.all('radio_saved_tracks'); for (const track of tracks.filter((item) => trackIds.includes(String(item.id)) && item.returnPrompt)) await db.put('moments', { id: `soundtrack-return-${detail.id}-${track.id}`, type: 'journal', title: `Return prompt · ${track.title || track.id}`, note: track.returnPrompt, walkId: detail.id, soundtrackTrackId: track.id, placeName: track.placeName || null, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() }); window.dispatchEvent(new CustomEvent('journal-data-changed', { detail: { walkId: detail.id, soundtrack: true } })); walkSoundtrackStartedAt = null; });
