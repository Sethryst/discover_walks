...
async function renderLibrary() { const target = el('radioLibrary'); if (!target) return; const tracks = await db.all('radio_saved_tracks'); target.classList.remove('hidden'); target.innerHTML = tracks.length ? `<strong>Your saved soundtrack</strong>${tracks.map((track) => `<button type="button" data-radio-library-track="${escapeHtml(track.id)}">${escapeHtml(track.title || track.id)}<small>${escapeHtml((track.categories || ['saved songs']).join(' · '))}</small></button>`).join('')}` : '<p>No saved local tracks yet. Play an archive track and choose Save.</p>'; target.querySelectorAll('[data-radio-library-track]').forEach((button) => button.addEventListener('click', () => { const track = tracks.find((item) => String(item.id) === String(button.dataset.radioLibraryTrack)); if (track) void playTrack(track); })); }
...
function bind() {
  ...
  window.addEventListener('walk-started', () => { walkSoundtrackStartedAt = Date.now(); });
