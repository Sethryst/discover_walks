...
async function playNext() { if (state.current) recordRadioEvent(state.current, 'skipped'); const next = state.queue.shift() || chooseTrack(); if (!next) return playTrack(null); state.queue.push(chooseTrack()); if (!(await playJingle())) return; await playTrack(next); }
