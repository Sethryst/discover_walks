...
import { objectiveCost, ROUTE_OBJECTIVES } from '../js/planner.js';

test('objective costs prefer different route evidence', () => {
  ...
  assert.ok(objectiveCost(green, 'green') < objectiveCost(direct, 'green'));
  assert.equal(ROUTE_OBJECTIVES.find((item) => item.key === 'shade').dataStatus, 'placeholder');
});
